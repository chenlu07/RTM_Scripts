...
public class ModelCrowdMapManager : MonoBehaviour {
    ...
    public static int[]  _agentclothes;
    public static GameObject[] _sidewalks;
	public static Stack<int> _numbers; //available numbers, based on crowdsize
    public static int crowdtype = 1;
    public static int staticdensitycrowdpointnum = 5;
    public static int staticdensitycrowdgroupnum = 7;
    public float staticdensitycrowdradius = 0.45f;
	...
	protected void BuildCrowd(){
		if (crowdsize <= 0) {
			throw new System.ArgumentException("Crowd must be bigger than 0");
		}
        float []x_location =new float[7] { 1.35f,0.85f,0.35f,0.0f,-0.35f, -0.85f,-1.35f};
        float []z_location = new float[11] {-1.223f,-0.9611f, -0.7373f, -0.5174f, -0.2726f, -0.0199f, 0.1998f, 0.4393f, 0.6858f, 0.9103f, 1.131f };
		...
		for (int n = 0; n < crowdsize; n++)
		{
            int x_index = n / 11;
            int z_index = n % 11;
            Vector3 m_pos = new Vector3(x_location[x_index]*35, 0, z_location[z_index] * 30);

            if (n == 0)
			{
                //Instantiate first agent randomly on sidewalk
                _characters[n] = this.SpawnRandom(m_pos);
	...
	public void Respawn(GameObject character){
        ...
        character.transform.position.Set(10.32089f,0, 28.42488f);
	 ...
	 */
	protected GameObject SpawnOutside(Vector3 m_pos){
		GameObject randomSidewalk = _sidewalks[Random.Range (0, _sidewalks.Length)]; //get random sidewalk square
        ...
        GameObject character = (GameObject)Instantiate (FullCharacter, m_pos, randomSidewalk.transform.rotation);
    ...
    protected GameObject SpawnRandom(Vector3 mpos)
    {
        GameObject character = (GameObject)Instantiate(FullCharacter, mpos, Quaternion.identity);
    ...
    protected GameObject generateWanderPoint(Vector3 m_pos)
    {
        GameObject prefabwander = (GameObject)Instantiate(wander, RandomPoint(m_pos, 0.2f), Quaternion.identity);
        return prefabwander;
    }

    protected GameObject generateWanderPointWithoutRandom(Vector3 m_pos)
    {
        GameObject prefabwander = (GameObject)Instantiate(wander, m_pos, Quaternion.identity);
        return prefabwander;
    }
    ...
    protected void BuildCrowdFordensity()
    {
        if (staticdensitycrowdpointnum * staticdensitycrowdgroupnum <= 0)
        {
            throw new System.ArgumentException("Crowd must be bigger than 0");
        }
        _characters = new GameObject[staticdensitycrowdpointnum* staticdensitycrowdgroupnum];
        LayerMask alayer = LayerMask.NameToLayer("Character");

        for (int i =0; i<= staticdensitycrowdpointnum; i++)
        {
            Vector3[] crowdpoint = new Vector3[staticdensitycrowdpointnum];
            crowdpoint[i] = RandomPoint(new Vector3(0, 0, 0), 50.0f);

            for (int j = 0; j < staticdensitycrowdgroupnum; j++)
            {
                int n = staticdensitycrowdgroupnum * i + j;
                Vector3 wanderpoint = RandomPoint(crowdpoint[i], 0.1f);

                if (n == 0)
                {
                    //Instantiate first agent randomly on sidewalk
                    _characters[n] = this.SpawnRandom(wanderpoint);
                    _characters[n].tag = "Player";
                    _characters[n].layer = 14;
                ...
                if(n% staticdensitycrowdgroupnum == 0)
                {
                    //Add character label
                    GameObject canvas = Instantiate(ObjectLabel) as GameObject;
                    canvas.transform.parent = _characters[n].transform;
                ...
                //Generate new wanderpoints
                GameObject newwander1 = generateWanderPointWithoutRandom(wanderpoint);
                newwander1.name = "wanderpoint1";
                ...
                //Set wanderbehavior references to new points
                _characters[n].GetComponent<WanderBehavior>().wander1 = newwander1.transform;
                _characters[n].GetComponent<WanderBehavior>().wander2 = newwander1.transform;
                _characters[n].GetComponent<WanderBehavior>().wander3 = newwander1.transform;
    ...
    public static int GetNumOfAgentsindistance(Vector3 m_centeragentpos,float m_dis)
    {
        int m_num = 0;
        int m_maxnum = 0;
        int m_crowdsize = crowdsize;
        if (crowdtype == 0)
        {
            m_maxnum = crowdsize;
        }
        ...
        {
            m_maxnum = staticdensitycrowdpointnum * staticdensitycrowdgroupnum;
        }
        for(int i = 0; i< m_maxnum; i++)
        {
            if (Vector3.Distance(m_centeragentpos, _characters[i].transform.position) < m_dis)
                m_num++;
        }
